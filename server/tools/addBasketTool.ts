...
export async function addBasket(walletUUID: string, productIDs: number[], event: any) {
  console.log('💬 [addBasket] Executing with:', { walletUUID, productIDs })
  ...
  try {
    ...
    console.error('Error in addBasket:', err)
    return `❌ Error in addBasket: ${err.message}`
  }
}